...
#     ARTICLE + ('PublicationTypeList', 'PublicationTypes'): 'pubtypes',
      ...
      ROOT + ('MedlineJournalInfo', 'Country'): 'country'
   }
   ...
   def json_wrap(self):
      ...
      abstr['volume'] = '_PARSE_ERROR_'.join(self._dict['vol'])
      abstr['country'] = '_PARSE_ERROR_'.join(self._dict['country'])
      ...
      abstr['year'] = '_PARSE_ERROR_'.join(self._dict['year'])
      abstr['pubdate'] = ' '.join(
         self._dict['month'] \
         + self._dict['day'] \
         + self._dict['year']
      ...
      abstr['MESH'] = ', '.join([i.replace(',','') for i in self._dict['mesh']])
